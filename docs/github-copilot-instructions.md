...
12b. **Warningy čti jako chyby.** `status: "ok"` + `warnings` = zapsáno, ale **část záměru se nepropsala** (`scenarios[1]: join 'BE95002' neni cislo kroku …`). Chat ACK nese počet + první warning v prvním řádku, plný výčet je v `res-<id>.json`. Řeš opravnou dávkou dle textu warningu, nikdy přeposláním.
