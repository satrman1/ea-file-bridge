...
2. **Chat ACK je jen výcuc, ne jediný záznam.** `EAFB OK <id>: N/N ops | QC ciste` = hotovo; `EAFB OK <id>: N/N ops | 1 WARNING: … | QC …` = zapsáno, ale část záměru se nepropsala (viz pravidlo 12b). Plná odpověď (GUIDy, aiLogGuid, řádky query, plný výčet warningů) žije v `res-<id>.json` — když potřebuješ GUIDy pro navazující dávku, požádej uživatele o obsah `res` souboru.
...
12b. **Warningy čti jako chyby.** `status: "ok"` + `warnings` = zapsáno, ale **část záměru se nepropsala** (`scenarios[1]: join '2' neni v davce - Join nezapsan`). Chat ACK nese počet + první warning v prvním řádku, plný výčet je v `res-<id>.json`. Řeš opravnou dávkou dle textu warningu, nikdy přeposláním.
12c. **Dvě tiché pasti bez warningu:** (a) **`join` = JMÉNO scénáře**, kam se tok vrací, ne číslo kroku — EA neumí návrat na konkrétní krok; (b) **přesun elementu mezi packages bridge neumí** — `package` v update větvi je bez účinku a vrátí `status: "ok"`, `created: false` (**falešné OK**); přesun vykaž jako ruční krok a kontroluj `PackageID` zpětným čtením.
