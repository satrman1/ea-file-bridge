...
- **Output proklik (V1):** `Log(Repository, msg, id)` má volitelný 3. parametr — ElementID putuje do `WriteOutput`. `FB_LogChanges` předává ElementID vytvořených/upravených prvků (package přes `Package.Element.ElementID`; smazané id 0 — není kam skočit). Funguje ve všech kanálech vč. pumpy. **⚠ DVĚ KOREKCE ze živého E2E 2026-08-20 (K3):** (1) tvrzení GUI-KATALOGU §5 „dvojklik naviguje nativně" pro CUSTOM tab **neplatí** — EA u custom Output tabu sama nenaviguje, dvojklik jen vysílá broadcast; navigaci dělá nový handler **`EA_OnOutputItemDoubleClicked`** (`ShowInProjectView` z **user-gesture kontextu** = živý test spike hypotézy b1; vypínač `FB_Config.outputNav: false` pro případ pádu §1a/4). (2) Broadcast handler musí být **RECEPTION**: `t_operation` se `StyleEx = 'Reception=1;SignalGUID={GUID Signalu z Broadcast Types};'` — obyčejná operace event nedostane; `deploy_src` od v0.10 reception zakládá/doplňuje sám (jméno operace = jméno Signalu v modelu, response `receptions`). Signal `EA_OnOutputItemDoubleClicked` v eaexample = `{5F05064B-FABB-4e13-A447-46D60927340B}` (el. 10303).
