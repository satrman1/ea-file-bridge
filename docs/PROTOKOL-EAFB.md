...
- **v0.10 (iterace 5, §6g/B):** řádky změn v Output tabu jsou **dvojklikem navigovatelné** na prvek v browseru (nativní chování EA, ElementID ve `WriteOutput`); Add-in Search **`FB_Changes`** ukáže všechny dotčené prvky dávky jako výsledkovou sadu (dvojklik = skok); `FB_ShowInBrowser` dál default vypnuto (do závěru spiku `FB_NavProbe`).
