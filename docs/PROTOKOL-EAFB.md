...
| `E_SQL` | op | **nové v 0.13** (nález N2 E2E VS Code 2026-09-07): `query` selhal v EA — neexistující sloupec/tabulka, syntaktická chyba; `message` = první řádek hlášky EA (např. `SQL API Open FAILED: no such column: Type`), `sql` = dotaz (ořez 500). Detekce: výsledek `SQLQuery` bez `<Dataset_0>` (prázdný řetězec / chybový text / `<EADATA>` bez datasetu) nebo výjimka; **legitimní 0 řádků má `<Dataset_0>` a zůstává `ok/rowCount 0`**. Kolem dotazu `SuppressEADialogs = true` → žádný modální dialog v EA. Řešení: **oprav dotaz** (názvy sloupců ověř přes `sqlite_master`/`INFORMATION_SCHEMA`, §10) |
...
**Jak si ověřit názvy sloupců, než dotaz odešleš** (nález N-4, POC 2026-08-21 — pravidlo „ověř si sloupce" existovalo, ale bez nástroje). Neznámý sloupec v EA **nevrátí chybu** — otevře modální dialog, pumpa/vrátný visí do odkliknutí (§6b lekce, `Method_ID` vs `OperationID`) a po zotavení hrozí **falešná nula** (§5a/5). **Od v0.13 (N2 E2E VS Code 2026-09-07) platí pro operaci `query`:** chybný SQL vrací `error`/**`E_SQL`** s hláškou EA a dialog se díky `SuppressEADialogs` neotevře (§7) — pravidlo „ověř si sloupce" tím neztrácí smysl (stojí to dávku), jen přestává být jedinou obranou. Interní dotazy ostatních operací (`FB_ResolveEl`, `FB_OpDelete`, …) obal nemají — jsou to dotazy s pevnými, ověřenými sloupci.
