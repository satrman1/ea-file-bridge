...
**Nosič kódu (rozhodnutí 2026-08-19):** `-EncodedCommand` unese ~12 k znaků (Base64 z UTF-16 inflatuje ×2,67 pod strop 32767 — W2/R4), kanon vrátného má ~15,5 k. Místo fallbacku R1 (session mód) se zvolil **StdIn carrier**: EncodedCommand nese jen ~30znakový bootstrap `iex([Console]::In.ReadToEnd())`, plný kanon jde rourou. Drží všechny principy §2 (kanon v modelu, zrození inline z EA, žádný soubor na disku, `powershell.exe` 5.1); mění se jen nosič payloadu (roura místo command line) → bez délkového limitu. Odchylka od zadání §7/R4 „při překročení = R1" — zapsána i do oznámení R5.
