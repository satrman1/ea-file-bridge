...
- **T1 (Z260904-1, 2026-09-04) — proklik PER TYP artefaktu:** `FB_LogChanges` píše na konec řádku marker cíle `(el:ID)` | `(pkg:ID)` | `(dgm:ID)`; handler `EA_OnOutputItemDoubleClicked` ho parsuje z `LineText` a dispatchne `GetElementByID` / `GetPackageByID` / `GetDiagramByID` + `ShowInProjectView`. 3. parametr `WriteOutput` (nativní navigace EA) se posílá **jen u elementů** — PackageID/DiagramID žijí v jiném číselném prostoru. Mapování řádků Output → cíl prokliku:
  ...
  | `[vytvoreno\|upraveno] package "P" @ cesta` | `(pkg:PackageID)`, 3. param 0 | package | jen handler (marker → `GetPackageByID` → `ShowInProjectView`) | ❌ **nic se neoznačí** — příčina nerozhodnuta (debug v6 nasazen, viz `docs/e2e-pumpa/VYSLEDKY-2026-09-05.md` N1) |
  | `[diagram vytvoren\|upraven] "D"`, `[place_elements_on_diagram\|remove_elements_from_diagram\|layout_connectors\|update_diagram_properties\|set_diagram_object_style] hotovo` | `(dgm:DiagramID)`, 3. param 0 | diagram | jen handler | ❌ totéž |
...
- **Před každým deployem: `node test/harness.js`** (offline sito, mock COM/EA — syntaxe všech `src/` + funkční testy; aktuálně 221/221 PASS, 2026-09-05). Harness **nenahrazuje** živý E2E: dual-runtime pasti §1a/4 mock nechytí.
