...
2. **Chat ACK je jen výcuc.** `EAFB OK <id>: N/N ops | QC ciste` = hotovo; plná odpověď (GUIDy, `aiLogGuid`, řádky, **plný výčet warningů**) je v `res-<id>.json` — pro navazující GUIDy si vyžádej obsah `res` souboru.
...
12b. **Warningy čti stejně pozorně jako chyby.** Operace může vrátit `status: "ok"` a přitom nést `warnings` — zápis proběhl, ale **část záměru se nepropsala** (např. `scenarios[1]: join '2' neni v davce - Join nezapsan`). Chat ACK nese počet a první warning v prvním řádku; **plný výčet je v `res-<id>.json`** — u dávek se scénáři, constrainty a zprávami si ho vyžádej. Warning řeš opravnou dávkou dle jeho textu.
12c. **Dvě tiché pasti, které nehlásí ani warning:**
   - **`join` u větví scénářů = JMÉNO scénáře**, kam se tok vrací (ne číslo kroku). EA neumí návrat na konkrétní krok — návrat formuluj na úrovni scénáře. Číslo v `join` skončí warningem a větev zůstane bez návratu.
   - **Přesun existujícího elementu mezi packages bridge NEUMÍ.** Pole `package` v update větvi `create_or_update_elements` je bez účinku a operace vrátí `status: "ok"`, `created: false` — **falešné OK**. Potřebuje-li cílový stav přesun, **vykaž ho uživateli jako ruční krok v EA browseru** a nevydávej ho za hotový. Kontrolní čtení po zápisu ověřuj i na `PackageID`.
