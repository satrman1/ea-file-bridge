...
| N2 | 8a | Dotaz `SELECT … Type AS ConstraintType FROM t_objectconstraint` — sloupec se jmenuje `ConstraintType`. EA otevřelo modální dialog „SQL API Open FAILED: no such column: Type" (Miloš musel odkliknout), bridge vrátil `ok, rowCount: 0`. Stálo 1 dávku (E8). | šablona + **bridge** | ✅ `eafb-bridge/SKILL.md` (sloupce QA tabulek, falešná nula), `e2e-f0-f1` kolo 8; **bridge ✅ 8. 9. (Z260908-1, v0.13)**: `query` vrací `error`/`E_SQL` s hláškou EA, `SuppressEADialogs` kolem `SQLQuery` → žádný dialog; živé ověření dávky V1/V3 (`OVERENI-S-OPRAVY-2026-09-08.md`) |
