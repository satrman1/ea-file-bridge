...
- **(c) nosič kódu (StdIn carrier)**: kanon 15,5 k > `-EncodedCommand` ~12 k (W2/R4 potvrzen) → EncodedCommand nese jen bootstrap, kanon rourou (`WScript.Shell.Exec` = CreateProcess). Drží principy §2; odchylka od §7/R4 „R1 fallback" zapsána do oznámení R5.
