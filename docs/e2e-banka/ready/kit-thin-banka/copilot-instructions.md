...
1. **Repo povinné.** Každá dávka nese `"protocol": "eafb/0.2"`, `"repo": "<TEST-DB>"` a `id` (krátké, unikátní v rámci dne, např. `20260910-03`; pole `requestId` neexistuje).
...
3. **Zápis jen do whitelistu:** #FB-TEST <GUID-SANDBOX>. `E_WHITELIST` / `E_REPO` / `E_OP_FORBIDDEN` / `E_ADDIN_ACCESS` / `E_PERMISSION` neobcházej — ohlas.
4. **Zakázané operace v tomto prostředí:** deploy_src, delete_from_model, clone_package. Nenavrhuj je; co tím nejde, je ruční krok člověka v EA.
