...
2. Zapiš ji jako `requests/req-<id>.json` (`id` = krátké, unikátní v rámci dne, např. `20260910-03`). Repozitář: `<TEST-DB>`. Whitelist zápisu: #FB-TEST <GUID-SANDBOX>. Dialekt SQL: mssql. Zakázané operace: deploy_src, delete_from_model, clone_package.
...
- SQL jen SELECT/WITH; před dotazem si ověř názvy sloupců. **Chybný SQL (neexistující sloupec/tabulka, syntaxe) vrací `status: "error"`, `code: "E_SQL"`** (bridge v0.13; hláška EA v `message` být nemusí) — **oprav dotaz a pošli znovu**, nic jiného. EA přitom otevře modální dialog, který člověk musí odkliknout — chybný dotaz tedy stojí dávku **i klik u stroje**, proto ověřuj sloupce PŘED odesláním. U nejistého sloupce dej `SELECT TOP N *` / `SELECT * … LIMIT N` (podle dialektu). **Prázdný výsledek = `status: "ok"`, `rowCount: 0`** (od bridge v0.14, MS SQL vrací obálku bez datasetu) a typicky nese warning „EA nerozlišuje 0 řádků od chybného dotazu“ — **není to chyba**, nepřerušuje dávku a nečti ho jako `E_SQL`; znamená „dotaz nic nevrátil“, ne nutně „data neexistují“ (chybný dotaz poznáš podle dialogu v EA u stroje, ne z odpovědi). Sloupce, které QA čte nejčastěji: `t_objectscenarios (Object_ID, Scenario, ScenarioType, Notes, XMLContent)` · `t_objectconstraint (Object_ID, Constraint, ConstraintType, Notes, Status)` · `t_objectrequires (Object_ID, Requirement, ReqType, Notes, Status)` · `t_objectproperties (Object_ID, Property, Value)` · `t_diagramobjects (Diagram_ID, Object_ID)`.
- **Každý `SELECT` přes `query` má omezení: `TOP N` / `LIMIT N` podle dialektu mssql (`mssql` = `SELECT TOP N …`, `sqlite` = `… LIMIT N`) nebo selektivní `WHERE` (ID, GUID, `Package_ID`, jméno).** Velké tabulky (`t_seclocks`, `t_object`, `t_connector`, `t_diagramobjects`, `t_xref`, `t_attribute`, `t_operation`) **nikdy bez omezení** — v bance má `t_seclocks` přes milion řádků, EA sestavuje XML minuty a pumpa „visí“ na `Zpracovavam` (vypadá to jako pád). Řády velkých tabulek najdeš v kontextu repozitáře (`EA-Repozitar-Kontext.md`, v bance `EA-Repozitar-Kontext-banka.md`) — přečti ho před prvním reconem; není-li ve workspace, vyžádej si ho. Počty ber přes `COUNT(*)`, ne výpisem.
...
- Odpověď `res-<id>.json` je odsazený JSON (od bridge v0.13; starší pumpa píše jeden řádek) — čti soubor celý; když ho nástroj uřízne, zopakuj dotaz kompaktně (méně sloupců, menší `TOP N`/`LIMIT N`), ne celou dávku.
