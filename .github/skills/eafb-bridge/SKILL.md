...
- SQL jen SELECT/WITH; před dotazem si ověř názvy sloupců; `rowCount: 0` ≠ „data neexistují". **Chybný SQL (neexistující sloupec/tabulka) vrací `status: "error"`, `code: "E_SQL"`** (bridge v0.13; hláška EA v `message` být nemusí) — **oprav dotaz a pošli znovu**, nic jiného. EA přitom stále otevře modální dialog, který člověk musí odkliknout — chybný dotaz tedy stojí dávku **i klik u stroje**, proto ověřuj sloupce PŘED odesláním. U nejistého sloupce dej `SELECT *`. Sloupce, které QA čte nejčastěji: `t_objectscenarios (Object_ID, Scenario, ScenarioType, Notes, XMLContent)` · `t_objectconstraint (Object_ID, Constraint, ConstraintType, Notes, Status)` · `t_objectrequires (Object_ID, Requirement, ReqType, Notes, Status)` · `t_objectproperties (Object_ID, Property, Value)` · `t_diagramobjects (Diagram_ID, Object_ID)`.
...
- Odpověď `res-<id>.json` je odsazený JSON (od bridge v0.13; starší pumpa píše jeden řádek) — čti soubor celý; když ho nástroj uřízne, zopakuj dotaz kompaktně (méně sloupců, `LIMIT`), ne celou dávku.
