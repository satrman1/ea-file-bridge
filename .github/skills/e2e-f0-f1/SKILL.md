...
| 2 | `/prevzeti-zadani` | Přečti zadání ze `zadani/*.md` (cesta z argumentu, jinak jediný soubor ve složce). Vyextrahuj požadavky (ID, název, popis, akceptační kritéria), navrhni hranici řešení a projekt. Je-li `login` z pingu prázdný (EA security vypnutá), zeptej se v téže HUMAN_DECISION na hodnotu razítka `AI-User`. → **HUMAN_DECISION** v chatu; dokud není schváleno, žádná dávka. | — |
| 3 | `/eafb-bridge` | Zápis: **jedna** package `Business Requirements` pod whitelist větví (existuje-li projektová package, pod ní) + Requirement elementy `<ID> Název` (notes = popis + AK) řetězené přes `$N` v téže dávce. Jedna nová package na dávku = LOW — **bez `matchByName`** (cíle podle zadání neexistují; `matchByName` na nové package udělá z řetězu „nejistý původ" a Risk Gate dávku zvedne na ELEVATED). GUIDy z ACK si zapiš. | 2 (LOW) |
| 4 | `/emr-qa` sada F0 | Čtecí dávka (packages, elementy) + LLM kontrola; do téže čtecí dávky přibal `query` na nejvyšší obsazené `UC-#####` v cílové větvi, existující aktéry a **typ diagramu existující UC package ve větvi** (vzor typování pro kolo 6) — ušetří dávku. QA report Artifact do `#QA` (package `#QA` založ v téže dávce, neexistuje-li; bez `matchByName`). Blokující nález → oprav (opravná dávka) před pokračováním. | 3 (čtecí), 4 (LOW) |
...
| 6 | `/emr-scaffold` (režim B) + `/use-case-model` | Číslo UC = nejvyšší obsazené `UC-#####` v cílové větvi (`query`) + 1; vykaž „posunout Auto Name Counter" jako ruční krok. **Jedna řetězená dávka `$N`**: package `UC-##### Název` → UseCase (kompozitní diagram `CSOB-ITAN::FA-Behavioral` pod UC) + `UCR-#####` (typ `Use Case Realization`) + diagram `version_UC-##### Název` (`CSOB-ITAN::Version Root Diagram`, rovnou naplněný) + Aktér (existující z ACTORS, jinak založ) —Association→ UC, UCR —Realization→ UC, `place_elements_on_diagram` + Boundary. **Právě 1 nová package** (víc packages = ELEVATED navíc), bez `matchByName`. Typování: vzor v modelu > pravidla — vzor = existující UC package v cílové větvi (`get_packages_information` → typ jejího diagramu, např. `CSOB-ITAN::FA-Behavioral`). MDG typ **není** v `t_object.Object_Type` ani `t_diagram.Diagram_Type` (tam je vždy jen základní UML typ) — dotaz na `'%::%'` v těchto sloupcích MDG neodhalí; kvalifikovaný typ zadávej i tak, bridge zapíše `MDGDgm`. Fallback na základní UML typ + stereotyp jen když ve větvi žádný vzor není. | 5 (LOW) |
| 7 | `/eafb-bridge` | `create_or_update_scenarios` (kompletní sada BE/AF/EF, `attachTo` + `join` = **číslo kroku**), `create_or_update_constraints` (PRE/PST/ASU), `create_or_update_requirements` (lokální BRU jako internal requirements), konektory UC —Realization→ požadavky (traceabilita) — v jedné dávce. **ELEVATED** (operace `scenarios` / `constraints` / `requirements` jsou politikou ELEVATED vždy — proto je právě tady jediné potvrzení běhu): dávka skončí `EAFB CEKA NA POTVRZENI`, člověk klikne Ano v EA; ty čekáš na finální ACK. Pak zkontroluj **warningy** (join, typy) — oprava = rebuild kompletní sady na element GUID. | 6 (ELEVATED) |
| 8 | `/emr-qa` sada F1 | Čtení (`t_objectscenarios` — `Scenario`, `ScenarioType`; `t_objectconstraint` — `Constraint`, `ConstraintType`; `t_objectrequires` — `Requirement`, `ReqType`, `Notes`; konektory, obsah package, diagramy) + report Artifact do `#QA`. Chybný název sloupce = `error`/`E_SQL` (oprav dotaz) a navíc modální dialog v EA, který odklikne Miloš — u nejistého sloupce `SELECT *`. Očekávaný verdikt: pass s varováními, jediné B/W = kontroly 6/6b (LS mimo řez). Cokoli jiného → opravná dávka. | 7 (čtecí), 8 (LOW) |
| 9 | — | Podklad G1 (gate package lite v chatu: confidence flags, QA verdikt, co vzniklo s GUIDy a plnými cestami, tier) → **HUMAN_DECISION** → Gate Record Artifact do `#GATES` (TV `gate=G1`, `datum`, `rozhodl`, `vysledek`, `tier`; package `#GATES` založ v téže dávce, neexistuje-li). | 9 (LOW) |
