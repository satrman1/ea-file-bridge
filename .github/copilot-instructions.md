...
9. **SQL jen SELECT/WITH**, dialekt **sqlite**; před dotazem ověř názvy sloupců — chybný SQL vrátí `error`/`E_SQL` (oprav dotaz, pošli znovu) a navíc otevře v EA modální dialog, který musí odkliknout člověk; `rowCount: 0` s `ok` = dotaz skutečně nic nenašel.
