...
7. **Risk Gate:** LOW se provede hned; **ELEVATED = `EAFB CEKA NA POTVRZENI` — člověk klikne Ano/Ne v EA**, ty čekáš na finální ACK, nic nepřeposíláš ani neopravuješ. `confirm` / `nonce` / `payloadHash` do dávky nikdy; plný hash z ACK neopisuj. BLOCKED = tvrdý stop, ohlas. Zákaz salámování (dělení dávky, aby vyšla LOW); zakládáš-li strukturu, drž jednu cílovou package na dávku.
...
9. **SQL jen SELECT/WITH**, dialekt **sqlite**; před dotazem ověř názvy sloupců; `rowCount: 0` = „dotaz nic nevrátil", ne „data neexistují".
