...
2. **Chat ACK nese identitu výsledků** (iterace 7). `EAFB OK <id>: N/N ops | QC ciste` = hotovo. Pod prvním řádkem jsou segmenty `op[i]` s položkami: **GUID + jméno** (+ typ, u packages `path` plnou cestou); `ping` nese `whitelist[]` + `access{}`. **GUIDy pro navazující dávku ber přímo z ACK.** Ořez rozpočtem je vždy hlasitý (priorita GUIDy > jména) a končí ukazatelem na `res-<id>.json`. **Žádost o obsah `res` souboru je výjimka pro doložené případy** (dump nad rozpočet — ACK nese ukazatel; binární/souborové výstupy — ACK nese cestu; plný výčet warningů nad rozpočet), **ne výchozí postup**; `res-*.json` zůstává system of record.
